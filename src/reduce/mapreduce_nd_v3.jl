...
# Detects whether matrix should be transposed
function should_transpose(A, Rreduce, Rother)
...
"""
function mapreducedim_v3!(f::F, op::OP, R::AnyGPUArray{T}, A::AbstractArrayOrBroadcasted;
    ...
    do_transpose, reduce_dims = should_transpose(A, Rreduce, Rother)

    if do_transpose
        # permute dims so that reduced dims are first
        # println("transposing")
        all_dims   = 1:ndims(A)
        ...
        R_t = similar(R, R_size_t)
        init !== nothing && fill!(R_t, init)

        # reduce on transposed array — dim 1 is now the reduction dim
        mapreducedim_v3!(f, op, R_t, A_t; init=init)

        # transpose back
        copyto!(R, reshape(R_t, size(R)))
        return R
    end
    ...
    if reduce_groups == 1
        ...
        mapreducedim_v3!(identity, op, R, partial; init=init)  # Recursive call to self
    end
